...
module Clisms {
  ...
  if ((fs['existsSync'] || path['existsSync'])(CONF_FILE)) {
    CONF = JSON.parse(<string> fs.readFileSync(CONF_FILE, 'utf8'));
  }
  ...
  // exits.
  function shell(cmd: string, opts: string[], callback: (number) => void): void {
    var child;
    process.stdin.pause();
  ...
  function http_cmd(cmd: string, process_response: (string) => void): void {
    var query, url;
  ...
  function sanitize_phone_number(num: string): string {
    var result;
  ...
  function print_account_balance(): void {
  ...
  function send_message(text: string, to: string): void {
    var name, sender_id;
